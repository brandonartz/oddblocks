...
public class OddBlocksTier9Feature extends Feature<DefaultFeatureConfig> {
    private static final BlockPos START_BLOCK = new BlockPos(8, 3, 8);
    private static String identifier = "odd_tier9";
    private static final Feature<DefaultFeatureConfig> ODD_FEATURE = Registry.register(Registry.FEATURE, identifier, new OddBlocksTier9Feature(DefaultFeatureConfig.CODEC));  
    
    public OddBlocksTier9Feature(Codec<DefaultFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if(blockPos.isWithinDistance(START_BLOCK, 500)){
            return true;
        }

        Random random = new Random();
        Optional<Block> optional = Registry.BLOCK.getEntryList(OddBlocksMod.ODDBLOCKS_TIER9).flatMap(blocks -> blocks.getRandom(random)).map(RegistryEntry::value);
        if (optional.isEmpty()) {
            return false;
        }
